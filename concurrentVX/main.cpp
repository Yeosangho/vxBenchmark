...
using namespace cv;
	...
	vx_image src2 =  nvx_cv::createVXImageFromCVMat(context, mat);
	int a = 0;
	...
	vx_image gray2 = vxCreateVirtualImage(graph2, width, height, VX_DF_IMAGE_U8);
        vx_image blurred2 = vxCreateVirtualImage(graph2, width, height, VX_DF_IMAGE_U8);
        vx_image edges2 = vxCreateVirtualImage(graph2, width, height, VX_DF_IMAGE_U8);   
	...
	vx_node cvtNode2 = vxColorConvertNode(graph2, src2, gray2);
	NVXIO_CHECK_REFERENCE(cvtNode2);
	vx_node boxNode2 = vxBox3x3Node(graph2, gray2, blurred2);
	NVXIO_CHECK_REFERENCE(boxNode2);
	vx_node cannyNode2 = vxCannyEdgeDetectorNode(graph2, blurred2, CannyThreshold, 3, VX_NORM_L1, edges2);
	NVXIO_CHECK_REFERENCE(cannyNode2);
	...
	vxSetGraphAttribute(graph2, NVX_GRAPH_VERIFY_OPTIONS, option, strlen(option)); 
